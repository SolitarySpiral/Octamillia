...
0.1.6 Добавлена кафка, 2 паралельных вида транспорта сообщений в архитектуре. Тесты pytest не обновлялись.
Изменена реализация тентакли brokerage, добавлено сердце, изменена связь мозга с сердцем и транспортом сообщений.
...
В Octamillia: Контракт CommandDispatchTentacle содержит минимально необходимый набор методов (process_command, get_health, get_capabilities). Если бы мы добавили туда методы, нужные только для работы с БД, другие щупальца, не использующие БД, были бы вынуждены их реализовывать.
В текущей реализации создавать вручную методы process_command и get_capabilities не нужно
...
В текущей реализации щупальцу не нужно выполнять process_command, оно реализовано в архитектуре CommandDispatchTentacle.
