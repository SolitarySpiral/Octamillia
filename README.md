...
Шаблон присоски щупальца(универсальный шаблон для любого микросервиса)
Общение по артериям (обмен данными между микросервисами)
Тело
Пред-жопие
Жопа (база данных)
Интеграция с внешними микросервисами (external_image_tag) или работа с докером.
...
```python class ConfigLoaderStandinTentacle(CommandDispatchTentacle)``` реализует конкретную логику, не смотрит на начинку архитектуры.
